<?php
class Util {
	static function calcHash($u, $n = 32)
	{
		$h  = sprintf("%u", crc32($u));
		$h1 = intval($h / $n);
		$h2 = $h1 % $n;
	...
	static function isUserName($vStr)
	{
		return preg_match('/^[a-zA-Z][\w]{3,14}$/', $vStr);
	}

	static function isEmail($vStr)
	{
		return preg_match('/^\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/', $vStr);
	}

	static function isNickName($vStr)
	{
		return preg_match('/^[\x{4e00}-\x{9fa5}|\w]+$/u', $vStr);
	}

	static function isRealName($vStr)
	{
		return preg_match('/^([\x80-\xff\w])+$/', $vStr);
	}

	static function isAddress($vStr)
	{
		return preg_match('/^([\x80-\xff\w])+$/', $vStr);
	}

	static function isAnswer($vStr)
	{
		return preg_match('/^([\x80-\xff\w])+$/', $vStr);
	}

	static function isQQ($vStr)
	{
		return preg_match('/^[1-9]\d{4,8}$/', $vStr);
	}

	static function isPassword($vStr)
	{
		return preg_match('/^[\w|\!|\@|\#|\$|\%|\^|\&|\*|\~|\(|\)]{6,20}$/', $vStr);
	}

	static function isMobile($vStr)
	{
		return preg_match('/^1(3|4|5|8)\d{9}$/', $vStr);
	}

	static function isPhone($vStr)
	{
    ...
    static function isCaptcha($vStr)
	{
		return preg_match('/^[a-zA-Z0-9]{4,}$/', $vStr);
	}

	static function str_addslashes(&$_value)
	{
		if (!empty($_value))
		{
			if (is_array($_value))
			{
				...
				{
					Util::str_addslashes($_value[$_key]);
				}
			} else {
	...
	static function args_addslashes()
	{
		if (!get_magic_quotes_gpc())
		{
			Util::str_addslashes($_GET);
			Util::str_addslashes($_POST);
			Util::str_addslashes($_COOKIE);
		}
	}
	 ...
	 * @return boolean
	 */
	static function allowIP($iparray=array())
	{
		return in_array(Util::getIP(),$iparray);
	}
	 ...
	 * @return string
	 */
	static function createdealno()
	{
		return 'A'.date('YmdHis').mt_rand(10000,99999);
	}

	static function pageNavigator($total_info, $now_page, $page_size, $prefix, $suffix)
	{
		...
		{
			if ($now_page > 1)
			$nav_string .= "<a href=\"{$prefix}1{$suffix}\">首页</a>";

			if ($now_page > 1)
			$nav_string .= "<a href=\"{$prefix}" . strval($now_page - 1) . "{$suffix}\" >上一页</a>";
			...
			$nav_string .= "<a href=\"{$prefix}" . strval($now_page + 1) . "{$suffix}\">下一页</a>";

			if ($now_page < $total_page)
			$nav_string .= "<a href=\"{$prefix}{$total_page}{$suffix}\">尾页</a>";
		}
	...
	static function myExpressPage($total_info, $now_page, $page_size, $prefix, $suffix) {
		$total_page = ceil($total_info / $page_size);
		$now_page  = min($total_page, max(1, $now_page));
		$nav_starter = max(1, $now_page - 2);
		$nav_ender = min($total_page, $now_page + 2);
		...
		if($total_page > 1)
		{
			$nav_string .= "<tfoot><tr><td colspan=\"4\" align=\"center\"><div class=\"pager_outer\"><div class=\"pager_inner\"><p class=\"pager_cont\">";
...
//			$nav_string .= "<a href=\"{$prefix}" . strval($now_page - 1) . "{$suffix}\" >上一页</a>";
			...
			$nav_string .= "</p></div></div></td></tr></tfoot>";
		}
	 ...
	 * @return boolean
	 */
	static function upload($field_name, $new_name)
	{
		$max_size = 2048 * 1024;//2M
		$allow_ext = array('jpg', 'gif', 'jpeg');

		$file_name = $field_name['name'];
		$tmp_name = $field_name['tmp_name'];
		$file_size = $field_name['size'];
		...
		// 检查和创建上传目录
		$upload_dir = ROOT_PATH. 'upload' . DS . $now_year . DS . $now_month . DS . $now_day . DS;
		if(!file_exists($upload_dir))
		{
			if (!mkdir($upload_dir,0777,true)) return false;
		}
		...
		// 判断是否是允许的上传文件类型
		if(!in_array(strtolower($file_info["extension"]), $allow_ext)) return false;

		// 是否大于允许的文件大小
		if($file_size > $max_size) return false;

		// 生成新文件名
		if (file_exists($upload_dir . $new_name)) return false;

		// 移动文件到上传文件夹
		if(!@move_uploaded_file($tmp_name, $upload_dir . $new_name)) return false;

		return true;
	}

	static public function strSub($vString, $vLength, $vSuffix='')
	{
		$vReturn = '';
		$vOldLength = strlen($vString);

		for ($i = 0, $n = 0 ; $i < $vOldLength ; )
		{
			...
			if ($n + min(2, $tmpStep) > $vLength)
			{
				break;
			} else {
				$vReturn .= substr($vString, $i, $tmpStep);
				$i += $tmpStep;
				$n += min(2, $tmpStep);
			}
		}

		return ($i < $vOldLength) ? ($vReturn . $vSuffix) : $vReturn;
	}

	static public function checkICard($creditno)
	{
		$check_status = 0;
		if (strlen($creditno) == 18) {
			$_age_18 = mktime(0, 0, 0, intval(substr($creditno, 10, 2)), intval(substr($creditno, 12, 2)), 18 + intval(substr($creditno, 6, 4)));
		} else {
			$_age_18 = mktime(0, 0, 0, intval(substr($creditno, 8, 2)), intval(substr($creditno, 10, 2)), 18 + intval('19' . substr($creditno, 6, 2)));
		}

		if (time() < $_age_18)
			$check_status = 0;//未满18周岁
		else
			$check_status = 1;//年满18周岁

		return $check_status;
	}

    static public function mFristAndLast($y="",$m="")
    {
        if($y == '0000') $y=date("Y");
        if($m == '00') $m=date("m");
	...
	* @param 主题 $Subject
	* @param 邮件内容 $body
	* @param 接收人的邮件地址 $address
	* @param 接收人的姓名 $name
	*/
	static public function sendoneEmail($Subject = '', $body = '', $address = '', $name = '尊敬的用戶') {
		include_once('sendEmail.class.php');
		if ($Subject == '')
			die('Subject in not null');
		if ($body == '')
			die('body in not null');
		if ($address == '')
			die('email in not null');
		if (!Util :: isEmail($address))
			die('email must is email address');
		if ($name == '')
			die('name in not null');
		$mail = new sendEmail();
		$mail->IsSMTP();
		...
		$mail->Username = 'service@5mgame.com'; //用户名
		$mail->Password = 'boshuo823#912'; // 密码
		$mail->CharSet = "utf-8";
		$mail->SetFrom('service@5mgame.com', '我家游戏'); //发送者的用户名以及发送者姓名
		$mail->AddReplyTo('service@5mgame.com', '我家游戏');
		$mail->Subject = $Subject; //主题
		$mail->AltBody = "详细内容"; // optional, comment out and test
		$mail->MsgHTML($body);
		$mail->AddAddress($address, $name); //接收人的姓名
		//$mail->AddAttachment ("C:PHPMailer.zip"); // 添加附件也可以添加多个附件，修改附件名称

		$res = $mail->Send(); //发送
		unset ($mail); //一定要销毁该对象，否则附件会一直循环累加
		return $res;
	}

    static function smsrandstr() {
        $len=6;
        $chars='0123456789';
        mt_srand((double)microtime()*1000000*getmypid());
        $captcha='';
        while(strlen($captcha)<$len)
            $captcha.=substr($chars,(mt_rand()%strlen($chars)),1);
        return $captcha;
    }
    static function big2gb($s){
        $n=strlen($s);
