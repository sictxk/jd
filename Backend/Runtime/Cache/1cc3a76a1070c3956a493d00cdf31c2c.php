...
<head>
	...
	<script type="text/javascript">    
	    function DoDel(Pkid){
	    	var msg='确定要删除章节吗?';
	    	if(confirm(msg)){
	    		window.location.href="/Backend/Chapter/remove/pkid/"+Pkid ;
	    	}
	    }
	    function DoEdit(Pkid){
	      var url = "/Backend/Chapter/edit/pkid/"+Pkid+'/pno/'+<?php echo ($pNo); ?> ;
...
<body>
	...
	<form action="/Backend/Chapter/index" method="post" class="formskin">
	...
	<div class="container">
		<table border="1" cellpadding="5" cellspacing="0" width="100%" class="tableskin">
			...
			<?php if(is_array($lesson_video_list)): $i = 0; $__LIST__ = $lesson_video_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
				...
				<?php if($vo["status"] == 'Y'): ?><td align="center"><a href="/Backend/Chapter/hide/pkid/<?php echo ($vo["pkid"]); ?>"><img src="/Public/Backend/images/public/yes.gif" /></a></td>
				<?php else: ?>
				<td align="center"><a href="/Backend/Chapter/show/pkid/<?php echo ($vo["pkid"]); ?>"><img src="/Public/Backend/images/public/no.gif" /></a></td><?php endif; ?>
