...
<body>
	...
	<div class="container">
		<form id="MyForm1" name="MyForm1" method="post" action="/Backend/Chapter/save" enctype="multipart/form-data">
			<ul id="tabmenu">
				...
				<div class="tabitem tabitemselected">
					<table border="0" cellpadding="5" cellspacing="0" class="tableform" width="100%">
						<tr>
							...
							<td>
							<select id="lesson_id" name="lesson_id" onchange="" ondblclick="" class="" ><option value="" >选择教材</option><?php  foreach($arr_lesson as $key=>$val) { if(!empty($lesson_id) && ($lesson_id == $key || in_array($key,$lesson_id))) { ?><option selected="selected" value="<?php echo $key ?>"><?php echo $val ?></option><?php }else { ?><option value="<?php echo $key ?>"><?php echo $val ?></option><?php } } ?></select></td>
						</tr>
						<tr>
