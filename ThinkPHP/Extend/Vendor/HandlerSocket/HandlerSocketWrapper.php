...
class HandlerSocketWrapper {
	...
	function init($array = array(), $index = 1, $primary_key = NULL, $db_name, $db_table)
	{
		$this->index = $index;
	...
	function getall($key, $op = '=')
	{
		if( ! is_array($key))
			return $this->hs->executeSingle(1, $op, array($key), 100, 0);

		$array = array();
		foreach($key as $row)
		{
			$query = array($this->index, $op, array($row), 100, 0);
			array_push($array, $query);
		}
		return $this->hs->executeMulti($array);
	}
