...
class Captcha
{
    ...
    function showImg()
    {
        ...
        $this->setCaptcha();
        session_start();
        $_SESSION['session_captcha']= strtolower($this->code);
        //输出图片
        $this->outputImg();
    }
    ...
    private function createImg()
    {
        ...
        $bgColor = imagecolorallocate($this->im, 241, 241, 241);
    ...
    private function setCaptcha()
    {
        ...
        for ($i = 0; $i < $this->codeNum; $i++) {
            $color = imagecolorallocate($this->im, rand(0, 120), rand(20, 120), rand(68, 120));
            $size = rand(floor($this->height / 3), floor($this->height /1));
