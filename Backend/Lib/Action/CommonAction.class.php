...
class CommonAction extends Action {
	...
	public function chapter_select(){
		
		$lesson_id = $_GET['lesson_id'];
		
		$chapter = new Model("Chapter");
		$data_list = $chapter->query("Select pkid,title FROM chapter WHERE lesson_id=".$lesson_id." AND status='Y' ORDER BY pkid ASC ");		
		$this->assign('chapter_list',$data_list);
		$this->display('chapter_select');
	}
