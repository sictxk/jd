...
<body>
	...
	<div class="container">
		<form id="MyForm1" name="MyForm1" method="post" action="/Backend/Lesson/save" enctype="multipart/form-data">
			<ul id="tabmenu">
				...
				<div class="tabitem tabitemselected">
					<table border="0" cellpadding="5" cellspacing="0" class="tableform" width="100%">
                        <tr>
                            ...
                            <td><select id="grade_id" name="grade_id" onchange="" ondblclick="" class="" ><option value="" >选择年级</option><?php  foreach($grade_id as $key=>$val) { if(!empty($grade_id) && ($grade_id == $key || in_array($key,$grade_id))) { ?><option selected="selected" value="<?php echo $key ?>"><?php echo $val ?></option><?php }else { ?><option value="<?php echo $key ?>"><?php echo $val ?></option><?php } } ?></select>
                            <span id="course_panel"></span>
                            </td>
                        </tr>
						<tr>
						...
						<tr>
							<td align="right" width="100"><span class="need">讲师介绍：</span></td>
