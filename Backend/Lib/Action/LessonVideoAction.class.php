...
class LessonVideoAction extends Action {
    ...
    public function add(){
    	...
    	$this->assign('status', array('Y'=>'显示','N'=>'隐藏'));
        $this->assign('video_type', array('1'=>'正课','2'=>'练习'));
    	$this->assign('arr_lesson', $this->getLesson());
    ...
    public function edit(){
    	...
    	$this->assign('arr_lesson', $this->getLesson());
    	$this->assign('chapter_list', $this->getChapter($arr_form[0]['lesson_id']));
    	$this->assign('status', array('Y'=>'显示','N'=>'隐藏'));
        $this->assign('video_type', array('1'=>'正课','2'=>'练习'));

    	$this->assign('value', $arr_form[0]['status']);
        $this->assign('type_value', $arr_form[0]['video_type']);
        $this->assign('chapter_value', $arr_form[0]['chapter_id']);
    ...
    public function save(){
        ...
        if($document!=''){
        	...
        	$lesson_video->free_play='N';
		$lesson_video->add();
		$pno = $this->_param('pno');
			
    	redirect("/Backend/LessonVideo/index/p/".$pno);
    }
    ...
    public function renew(){
		...
		$data['chapter_id'] = $this->_param('chapter_id');
	...
	$msg = mb_convert_encoding("OK","UTF-8","GB2312");
    	...
    	redirect("/Backend/LessonVideo/index/p/".$pno.'/lesson_id/'.$lessonId,1,$msg);
    }
    ...
    public function getChapter($lesson_id=''){
		$chapter = new Model("Chapter");
		//进行原生的SQL查询
		$data_list = $chapter->query("Select * FROM chapter WHERE lesson_id=".$lesson_id." ORDER BY pkid ASC ");
		$chapter_list = array();
		foreach($data_list as $key=>$val){
			$chapter_list[$val['pkid']] = $val['title'];
		}
		return $chapter_list;
    }
    ...
    public function setFree(){
		...
		$lesson_video->query("UPDATE lesson_video SET free_play='Y' WHERE pkid=".$pkid);
    	$msg = mb_convert_encoding("OK","UTF-8","GB2312");
    	$pno = $this->_param('pno');
    	$lessonId = $this->_param('lessonId');
    	redirect("/Backend/LessonVideo/index/p/".$pno.'/lesson_id/'.$lessonId,1,$msg);
    ...
    public function cancelFree(){
    	
    	$pkid = $this->_param('pkid'); 
    	
		$lesson_video = new Model("LessonVideo");
		
		$lesson_video->query("UPDATE lesson_video SET free_play='N' WHERE pkid=".$pkid);
